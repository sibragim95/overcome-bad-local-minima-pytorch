...
train_steps = 20000; T = train_steps; batch_size = 1024
...
# Initialize ANN parameters ON THE GPU
    w = Var(((h**(3))*torch.abs(Rn((M, 1, h)))).to(device), name='inner_weights', trainable=False)
    b = Var(((h**(3))*Rn((M, 1, h))).to(device), name='inner_biases', trainable=True)
    v = Var(((h**(-7/8))*torch.abs(Rn((M, h, 1)))).to(device), name='outer_weights', trainable=False)
    c = Var((Rn((M, 1, 1))).to(device), name='outer_bias', trainable=True)
...
for i in range(T):
    ...
    if (i%20==0):
        # Bring tensors back to .cpu() before calling .numpy()
        par1_v = [p.reshape(M, d_i).detach().cpu().numpy() for p, d_i in zip(par1, d[0:2])]
        par2_v = [p.reshape(M, d_i).detach().cpu().numpy() for p, d_i in zip(par2, d[2:4])]
        par3_v = [p.reshape(M, d_i).detach().cpu().numpy() for p, d_i in zip(par3, d[4:6])]
        
        g1_v = [p.grad.reshape(M, d_i).detach().cpu().numpy() for p, d_i in zip(par1, d[0:2])]
        g2_v = [p.grad.reshape(M, d_i).detach().cpu().numpy() for p, d_i in zip(par2, d[2:4])]
        g3_v = [p.grad.reshape(M, d_i).detach().cpu().numpy() for p, d_i in zip(par3, d[4:6])]
...
# Calculate final risk levels on the GPU, then pull to CPU for NumPy
